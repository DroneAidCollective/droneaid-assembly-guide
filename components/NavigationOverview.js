      ...
      {showOverview && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 bg-white rounded-t-xl shadow-2xl z-50 max-h-[80dvh] overflow-hidden flex flex-col app-width mx-auto"
          >
            ...
            >
              <div className="app-width mx-auto space-y-2">
                {steps.map((step, index) => (
                  <button
                    key={index}
                    ref={index === currentStep ? currentStepRef : null}
                    onClick={() => handleStepClick(index)}
                    className={`w-full text-left p-4 rounded-lg transition-all cursor-pointer ${getStepClass(
                      index
                    )}`}
                  >
